const _SaveLoadMultichoice = function () {

    _loadMultichoiceAnswerByQuestionId = (questionId) => { 
        let answerjson = UserResponse.load(questionId);
        answerjson.then((data) => {
            if (data) {
                _getAnswerId(data);
                const theAnswer = data[data.length - 1]["AnswerJson"]["multichoice_"]["answer"];
                const selectedItems = data[data.length - 1]["AnswerJson"]["multichoice_"]["answer"]["selectedItems"];
                ...
                const setCheckboxes = (() => {
                    ...
                    for (let item of iterator) {
                        ...
                        if (Object.is(item[0], "Other")) {
                            ...
                            document.getElementById(item[1]).parentNode.parentNode.parentNode.parentNode.nextElementSibling.children[3].value = data[data.length - 1]["AnswerJson"]["multichoice_"]["answer"]["othertext"];
                        }
                    }
                })();
            }
    ...
    return {
        save: () => {     
            ...
            //const mmm = new Map(createArrayFromMap);
            //console.log(mmm.has("Three by"));
        ...
        load: () => {
            this._loadMultichoiceAnswerByQuestionId(parseInt(document.getElementById("current_questionId").value));
            //_loadFromStorage = (() => {
